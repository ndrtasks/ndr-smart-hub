(() => {
  ...
  const ARABIC_FONT_URL='https://cdn.jsdelivr.net/gh/google/fonts@main/ofl/notonaskharabic/NotoNaskhArabic-Regular.ttf';
  ...
  async function templateBytes(){const parts=await Promise.all(TEMPLATE_PARTS.map(p=>fetch(p).then(r=>{if(!r.ok)throw new Error('template');return r.text()})));return bytesFromBase64(parts.join(''))}
  ...
  function setText(form,name,value,alignRight=false){if(!esc(value))return;try{const f=form.getTextField(String(name));f.setText(esc(value));if(alignRight&&window.PDFLib?.TextAlignment)f.setAlignment(window.PDFLib.TextAlignment.Right)}catch{}}
  function setCheck(form,name,on){try{const f=form.getCheckBox(String(name));on?f.check():f.uncheck()}catch{}}
